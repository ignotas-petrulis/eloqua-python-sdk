...
class EloquaException(Exception):
    ...
    def __str__(self):
        return "Eloqua API Error {}: {}".format(self.error, self.error_description)
...
class Eloqua(object):
    ...
    def authenticate(self):
        ...
        basic_auth = b64encode('{client_id}:{client_secret}'
        ...
        data = {
            ...
            'username': '{company}\\{username}'
                .format(company=self.company, username=self.username),
            'password': self.password
        }   

        resp = self.post('{login_url}/auth/oauth2/token'
            .format(login_url=LOGIN_URL), data, headers)
    ...
    def create_activity_export(self, name, fields=None, filter=None):
        ...
        resp = self.post('https://secure.p03.eloqua.com/API/Bulk/2.0/activities/exports', data, headers)
    ...
    def create_export_sync(self, synced_instance_uri, callback_url=None):
        ...
        headers = self.buildHeaders()
        response = self.post('https://secure.p03.eloqua.com/API/Bulk/2.0/syncs', data, headers)
        return response
    ...
    def make_request(self, **kwargs):
        ...
        logger.info(u'{method} response: {status} {text}'\
